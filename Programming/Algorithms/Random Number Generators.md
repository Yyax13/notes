...
1. __Information Loss when multiplying__: If you multiply by an even number in binary, you're effectively shifting left at least one bit
...
2. __Even Numbers isn't invertible modulo $2^n$__: Most fast hashes and PRNGs Algorithms operate modulo $2^{64}$ or $2^{32}$. A multiplication is reversible only if the multiplier is odd, this comes from number theory:
